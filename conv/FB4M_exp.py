...
def FB4Mexp(state, channel):
    ...
    # Index conversion
    trainlidx, trainlidx_eval = toidx(trainl, 0, state.neval)
    ...
    for epoch_count in xrange(1, state.totepochs + 1):
        ...
        # Negatives
        trainln = create_random_mat(trainl.shape, np.arange(state.Ntrain))
        trainrn = create_random_mat(trainr.shape, np.arange(state.Ntrain))
